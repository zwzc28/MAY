...
import (
	...
	"github.com/MAY/go-MAY/core/forkid"
	...
	lps "github.com/MAY/go-MAY/les/lespay/server"
	...
	"github.com/MAY/go-MAY/p2p/nodestate"
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light MAY peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light MAY handshake failed", "err", err)
		return err
	}
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light MAY peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light MAY response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light MAY message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light MAY message arrived", "code", msg.Code, "bytes", msg.Size)
