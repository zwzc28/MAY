...
// Verify that Client implements the MAY interfaces.
var (
	_ = MAY.ChainReader(&Client{})
	_ = MAY.TransactionReader(&Client{})
	_ = MAY.ChainStateReader(&Client{})
	_ = MAY.ChainSyncReader(&Client{})
	_ = MAY.ContractCaller(&Client{})
	_ = MAY.GasEstimator(&Client{})
	_ = MAY.GasPricer(&Client{})
	_ = MAY.LogFilterer(&Client{})
	_ = MAY.PendingStateReader(&Client{})
	// _ = MAY.PendingStateEventer(&Client{})
	_ = MAY.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	if err != nil {
		t.Fatalf("can't create new MAY service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == MAY.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != MAY.NotFound {
		t.Fatal("error should be MAY.NotFound")
	}
}
