...
// geth is the official command-line client for MAY.
package main
...
import (
	...
	"github.com/MAY/go-MAY/console/prompt"
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-MAY command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2021 The go-MAY Authors"
...
func prepare(ctx *cli.Context) {
	...
	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Geth on MAY mainnet...")
	}
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full MAY node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("MAY service not running: %v", err)
		}
