...
import (
	...
	math2 "github.com/MAY/go-MAY/common/math"
...
// newAlethGenesisSpec converts a go-MAY genesis block into a Aleth-specific
// chain specification format.
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only ethash is currently supported between go-MAY and aleth
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-MAY/pull/11039,
...
// newParityChainSpec converts a go-MAY genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-MAY and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	spec.Genesis.Seal.MAY.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.MAY.MixHash = (genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyMAYGenesisSpec represents the genesis specification format used by the
// Python MAY implementation.
type pyMAYGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyMAYGenesisSpec converts a go-MAY genesis block into a Parity specific
// chain specification format.
func newPyMAYGenesisSpec(network string, genesis *core.Genesis) (*pyMAYGenesisSpec, error) {
	// Only ethash is currently supported between go-MAY and pyMAY
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyMAYGenesisSpec{
