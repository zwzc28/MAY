...
// nodeDockerfile is the Dockerfile required to run an MAY node.
var nodeDockerfile = `
FROM MAY/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.MAY/keystore/ && cp /signer.json /root/.MAY/keystore/' >> geth.sh && \{{end}}
...
// an MAY node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new MAY node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.MAY"],
