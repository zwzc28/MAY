// Copyright 2017 The go-MAY Authors
// This file is part of go-MAY.
//
// go-MAY is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-MAY is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with go-MAY. If not, see <http://www.gnu.org/licenses/>.
...
import (
	...
	"github.com/MAY/go-MAY/accounts/keystore"
	"github.com/MAY/go-MAY/cmd/utils"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the MAY protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for MAY and the faucet")
...
type request struct {
	...
	Account common.Address     `json:"account"` // MAY address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an MAY light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // MAY protocol stack
	client *ethclient.Client   // Client connection to the MAY chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the MAY light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the MAY service: %w", err)
	}
...
// close terminates the MAY connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the MAY address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/MAY/go-MAY/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
...
// the uniqueness identifier (user id/username), username, avatar URL and MAY address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the MAY address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No MAY address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and MAY address to fund on
// success.
...
// returning the username, avatar URL and MAY address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the MAY address and profile URL.
	//
...
// authNoAuth tries to interpret a faucet request as a plain MAY address,
