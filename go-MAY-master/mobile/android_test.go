...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.MAY", new NodeConfig());
			node.start();
			...
			MAYClient ec = node.getMAYClient();
	...
	// signatures too. Regression test for go-MAY issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/MAY/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.MAY", "github.com/MAY/go-MAY/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "MAY", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
