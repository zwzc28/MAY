...
import (
	...
	"github.com/MAY/go-MAY/accounts/usbwallet/trezor"
...
// or whether the MAY app was not started on it.
...
// MAY address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.MAYAddress)
	if _, err := w.trezorExchange(&trezor.MAYGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.MAYTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.MAYTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
