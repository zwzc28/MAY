...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte MAY address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an MAY account at a particular block.
    type Account {
    ...
    # Transaction is an MAY transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an MAY account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an MAY account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an MAY block by number or by hash. If neither is
