...
import (
	...
	"github.com/MAY/go-MAY/eth/fetcher"
...
// support all the operations needed by the MAY chain protocols.
...
// newHandler returns a handler for all MAY chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("MAY handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("MAY peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerEthPeer(peer); err != nil {
		peer.Log().Error("MAY peer registration failed", "err", err)
		return err
	}
...
func (h *handler) removePeer(id string) {
	...
	if eth != nil {
		logger.Debug("Removing MAY peer")
		h.downloader.UnregisterPeer(id)
		h.txFetcher.Drop(id)

		if err := h.peers.unregisterEthPeer(id); err != nil {
			logger.Error("MAY peer removal failed", "err", err)
		}
	}
