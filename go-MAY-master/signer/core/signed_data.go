// Copyright 2019 The go-MAY Authors
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useMAYV = true // Default to use V = 27 or 28, the legacy MAY format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useMAYV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the MAY PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useMAYV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an MAY ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}MAY Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useMAYV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
